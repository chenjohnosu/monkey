...
class ThematicAnalyzer:
    ...
    def _extract_english_key_phrases(self, text: str, top_n: int = 20) -> List[str]:
        """Extract key phrases from English text."""
        if not self.nlp:
            return []
    ...
    # The rest of the class methods (identify_themes_with_nmf, identify_concept_network, etc.)
    # would need to be updated to handle multilingual content by using the appropriate
    # preprocessing methods based on document language
    ...
    def extract_common_keyphrases(self, min_doc_frequency: int = 3) -> Dict[str, Any]:
        """
        Extract common key phrases across all documents with language awareness.

        Args:
            min_doc_frequency (int): Minimum number of documents a phrase must appear in

        Returns:
            Dict[str, Any]: Results of key phrase extraction
        """
        documents = self._preprocess_documents()
        if not documents:
            return {"error": "No documents found in vector store"}
        ...
        for doc in en_docs:
            doc_phrases = self._extract_english_key_phrases(doc['text'], top_n=15)
            doc_id = doc.get('id', f"doc_{doc_counter}")
            phrases_by_doc[doc_id] = doc_phrases
            doc_counter += 1

        # Process Chinese documents
        for doc in zh_docs:
            doc_phrases = self._extract_chinese_key_phrases(doc['text'], top_n=15)
            doc_id = doc.get('id', f"doc_{doc_counter}")
            phrases_by_doc[doc_id] = doc_phrases
            doc_counter += 1

        # Process other language documents (treating as English)
        ...
        for doc_id, phrases in phrases_by_doc.items():
            # Count each phrase only once per document
            unique_phrases = set(phrases)
            phrase_doc_counts.update(unique_phrases)

        # Filter phrases by document frequency
        common_phrases = [phrase for phrase, count in phrase_doc_counts.items()
                          if count >= min_doc_frequency]

        # Categorize phrases by language
        en_phrases = []
        zh_phrases = []

        for phrase in common_phrases:
            # Check if phrase contains Chinese characters
            has_chinese = any('\u4e00' <= char <= '\u9fff' for char in phrase)
            if has_chinese:
                zh_phrases.append({
                    "phrase": phrase,
                    "documents": phrase_doc_counts[phrase]
                })
            else:
                en_phrases.append({
                    "phrase": phrase,
        ...
        en_phrases.sort(key=lambda x: x["documents"], reverse=True)
        zh_phrases.sort(key=lambda x: x["documents"], reverse=True)

        # Get metadata for documents containing each common phrase
        phrase_document_metadata = {}
        for phrase in common_phrases:
            # Find documents containing this phrase
            docs_with_phrase = [
                doc_id for doc_id, phrases in phrases_by_doc.items()
                if phrase in phrases
            ]

            # Get file names for these documents if available
            doc_names = []
            for doc_id in docs_with_phrase:
                # Try to find the original document by ID
                for doc in documents:
                    if doc.get('id') == doc_id:
                        if 'metadata' in doc and 'file_name' in doc['metadata']:
                            doc_names.append(doc['metadata']['file_name'])
                        elif 'metadata' in doc and 'file_path' in doc['metadata']:
                            doc_names.append(Path(doc['metadata']['file_path']).name)
                        else:
                            doc_names.append(f"Document {doc_id}")
                        break

            phrase_document_metadata[phrase] = doc_names
        ...
        return {
            ...
            'total_keyphrases': len(common_phrases),
            'english_keyphrases': en_phrases,
            'chinese_keyphrases': zh_phrases,
            'phrase_document_mapping': phrase_document_metadata
        }