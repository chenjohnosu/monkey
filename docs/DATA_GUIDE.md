5. Reprocess the documents with `/run grind <workspace> force`# Monkey Data Loading and Formatting Guide
...
2. **Word segmentation**: Install jieba for better Chinese word segmentation:
   ...
   pip install jieba
   ```
3. **Custom stopwords**: Add domain-specific Chinese stopwords to `lexicon/stopwords_zh.txt`
...
Monkey uses stopword files and sentiment lexicons in the lexicon directory to improve analysis quality:

- `lexicon/stopwords_en.txt`: English stopwords
- `lexicon/stopwords_zh.txt`: Chinese stopwords
...
4. For sentiment analysis, you can also customize:
   - `lexicon/chinese_positive.txt`: Add domain-specific positive sentiment words
   - `lexicon/chinese_negative.txt`: Add domain-specific negative sentiment words
...
3. Consider customizing the sentiment lexicons:
   ```
   # Edit these files to improve Chinese sentiment detection
   lexicon/chinese_positive.txt
   lexicon/chinese_negative.txt
   ```
