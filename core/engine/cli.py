...
class CommandProcessor:
    ...
    def _show_specific_help(self, topic):
        ...
        if topic == 'config':
            ...
            Examples:
              ...
              /config log off                    - Turn off all logging
            """)
    ...
    def _cmd_config(self, args):
        ...
        if subcommand == 'llm':
            ...
            if hasattr(self.config, 'export_to_env'):
                self.config.export_to_env()
                print("Configuration exported to environment variables")
            else:
                print("Config export to environment variables not supported")
    ...
    def _config_log(self, args):
        ...
        if not args:
            print("Usage: /config log [file|console|both|status|off|on|default]")
            return
        ...
        if subcommand == 'file':
            ...
            # First make sure logging is on
            LogManager.toggle_logging(True)
            ...
            if len(args) < 2:
                # If no filename provided, use the default log file
                log_file = LogManager.default_log_file
            ...
            # Reset to default configuration: log to default file, console off
            LogManager.toggle_logging(False)  # Console off
            LogManager.add_file_handler(LogManager.default_log_file)
            print(f"Logging reset to default: console OFF, file logging to {LogManager.default_log_file}")

        elif subcommand == 'off':
            # Turn logging off (console only - file logging remains active)
            LogManager.toggle_logging(False)
            print("Console logging turned OFF (file logging remains active)")

        elif subcommand == 'on':
            # Turn logging on
            LogManager.toggle_logging(True)
            print("Console logging turned ON")

        elif subcommand == 'status':
            # Show current logging configuration
            log_state = LogManager.get_logging_state()
            active = log_state.get('active', False)
            log_file = log_state.get('log_file')

            print(f"Console logging is currently: {'ON' if active else 'OFF'}")

            if log_file:
                print(f"File logging is active to: {log_file}")
            else:
                print("File logging is not active")
            ...
            print("Available subcommands: file, console, both, status, on, off, default")
